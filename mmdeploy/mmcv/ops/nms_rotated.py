...
@torch.jit.ignore
def ext_nms_rotate(bbox: Tensor, scores: Tensor, order: Tensor,
                   dets_sorted: Tensor, iou_threshold: float,
                   multi_label: int):
    from mmcv.utils import ext_loader
    ext_module = ext_loader.load_ext('_ext', ['nms_rotated'])
    return ext_module.nms_rotated(bbox, scores, order, dets_sorted,
...
class ONNXNMSRotatedOp(torch.autograd.Function):
    ...
    @staticmethod
    @torch.jit.script
    def script_forward(boxes: Tensor, scores: Tensor, iou_threshold: float,
        ...
        for batch_id in range(batch_size):
            for cls_id in range(num_class):
                ...
                if _boxes.shape[0] == 0:
                    continue
                valid_inds = torch.nonzero(valid_mask).squeeze(dim=1)
                _, order = _scores.sort(0, descending=True)
                dets_sorted = _boxes.index_select(0, order)
                box_inds = ext_nms_rotate(_boxes, _scores, order, dets_sorted,
                                          iou_threshold, 0)
    ...
    def forward(ctx, boxes: Tensor, scores: Tensor, iou_threshold: float,
        ...
        """
        return ONNXNMSRotatedOp.script_forward(boxes, scores, iou_threshold,
                                               score_threshold)
